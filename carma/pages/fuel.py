...
class State(rx.State):
    ...
    def add_fuel_record(self, form_data: dict):
        with rx.session() as session:
            company = session.exec(select(Company).where(Company.friendly_name == form_data['company'])).first() #This is a bit janky, using the friendly name to find the company again...

            new_fuel_record = Fuel(date=datetime.strptime(form_data['date'], "%d-%m-%Y"), #need to find a date picker
    ...
    def load_fuel_records(self) -> list[Fuel]:
        ...
        with rx.session() as session:
            self.fuel_records = session.exec(
                select(Fuel)
            ).all()
            ...
            rx.dialog.content(
                ...
                rx.form(
                    rx.vstack(
                        ...
                        rx.dialog.close(
                            rx.button("Add record", type="submit")
                        ),
                    ),
...
def fuel_table():
    return rx.table.root(
        ...
        rx.table.body(
            rx.foreach(
                State.fuel_records, show_fuel_item
            )
        ),
