...
class Fuel(CarmaBase, table=True):
    ...
    def add_fuel_record(date, milage, liters, price, company):
        with rx.session() as session:
            new_fuel_record = Fuel(
                date=date, milage=milage, liters=liters, price=price, company_id=company
            )
    ...
    def process_fuel_to_df(fuel_records: Sequence["Fuel"]):
        fuel_records_df = pd.DataFrame(
            [vars(fuel_record) for fuel_record in fuel_records]
        )
        ...
        fuel_records_df["km_driven"] = fuel_records_df["km_driven"].fillna(
            0
        )  # First entry has no previous mileage

        # Calculate price_per_liter
        fuel_records_df["price_per_liter"] = (
            fuel_records_df["price"] / fuel_records_df["liters"]
        )
        ...
        fuel_records_df["consumption"] = (
            fuel_records_df["km_driven"] / fuel_records_df["liters"]
        )
        fuel_records_df["consumption"] = (
            fuel_records_df["consumption"]
